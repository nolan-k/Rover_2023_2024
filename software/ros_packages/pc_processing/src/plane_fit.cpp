...
public:
	...
	CountObjects() : Node("plane_fit") {
		
		//Subscribe to the rosbag stream
		subscriber_ = this->create_subscription<PointCloud2>("truncated_point_cloud", 5, std::bind(&CountObjects::sub_callback, this, _1));
		
		//Republish to:
		publisher_ = this->create_publisher<PointCloud2>("bottles", 10);
